...
def create_results():
    ...
    data_structure = {
        ...
        "NLP methods explanations": {
            "Bias Detection": "Bias detection is a Natural Language Processing (NLP) technique that processes input text in the form of comments from human beings based on the provided input prompt.\n It then compares the output with a binary score of 0/1, indicating 'Not Hate Speech' or 'Hate Speech,' derived from the dataset. The categorizations include Hate Speech and Not Hate Speech, and the scoring range spans from 0% (does not match at all) to 100% (perfect match).Confluence Links: https://confluence.de.nortal.com/display/HTWSTUD/Benchmarks#Benchmarks-GermEval and https://confluence.de.nortal.com/display/HTWSTUD/Project+Approach#ProjectApproach-GermEval",
